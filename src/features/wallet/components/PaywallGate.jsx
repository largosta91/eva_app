  ...
  return (
    <div className="h-screen bg-[#09080f] flex flex-col overflow-hidden">
      <div className="text-center pt-12 pb-6 px-6">
        ...
        <p className="text-[#7a748f] text-sm mt-2">Para chatear necesitas creditos</p>
      </div>
          ...
          <div key={p.id} onClick={() => setSel(p.id)}
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                {p.best && <span className="bg-[#c9a84c] text-[#09080f] text-[10px] font-bold px-2 py-0.5 rounded-full">MAS POPULAR</span>}
              </div>
              <div className="text-sm text-[#7a748f] mt-0.5">
                💎 {p.credits} creditos {p.bonus && <span className="text-green-400">{p.bonus}</span>}
              </div>
            </div>
      ...
      <div className="p-5 pb-8">
        <button onClick={handleBuy} disabled={!sel}
          className={`w-full py-4 rounded-full font-semibold text-[15px] border-none transition-all duration-200 ${sel ? "bg-gradient-to-r from-[#c9a84c] to-[#f0d882] text-[#09080f] cursor-pointer" : "bg-[#1a1826] text-[#7a748f] cursor-default"}`}>
          {sel ? "Comprar ahora" : "Selecciona un pack"}
        </button>
      </div>
    </div>
  );
